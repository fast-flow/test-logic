...
const check = function check (settings) {
    ...
    checkItem = function (item) {
        ...
        // minLength
        if (typeof completeItem.minLength !== 'undefined' || typeof completeItem.maxLength !== 'undefined') {
            if (typeof completeItem.minLength !== 'undefined' && typeof completeItem.maxLength !== 'undefined') {
                if (settings.value.length >= Number(completeItem.minLength) && settings.value.length <= Number(completeItem.maxLength)) {
                    data.error = false
                }
                else {
                    data = returnError(data, completeItem, settings)
                ...
                if (typeof completeItem.maxLength !== 'undefined') {
                    if (settings.value.length > Number(completeItem.maxLength)) {
                        data = returnError(data, completeItem, settings)
                    }
                    else {
                        data.error = false
